...
parser.add_argument('--contentPath',default='Freeway1',help='path to train')
...
def styleTransfer(contentImg,styleImg,imname,csF):
    ...
    Im4 = wct.d4(csF4)
    # vutils.save_image(Im4.data.cpu().float(),os.path.join(args.outf,imname))
    # return
    ...
    Im3 = wct.d3(csF3)
    # vutils.save_image(Im3.data.cpu().float(),os.path.join(args.outf,imname))
    # return
...
import mmcv
mmcv.frames2video('samples', 'test.avi', fps=12)
